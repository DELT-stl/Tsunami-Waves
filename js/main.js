...
function getSupportedTransform() {
  ...
  var n = getSupportedTransform();
  if (!n) return;
  var t, i;
  $(document).ready(function() {
    t = $(".parallax"), i = $(window).height(), $(window).scroll(), t.parallaxInit()
  }), $(window).resize(function() {
    i = $(window).height(), t.parallaxInit()
  }), $.fn.parallaxInit = function() {
    this.each(function() {
      var t = $(this).outerHeight(),
        e = parseFloat($(this).data("speed")) || .5;
      t += $(this).outerHeight() * e, t = i + $(this).outerHeight() * (1 - e), $(this).find(".parallax-bg").css("height", t)
    })
  }, $(window).scroll(function() {
    window.requestAnimationFrame(e)
  });
  var a, r, o, s = document.getElementsByClassName("parallax"),
    m = s.length;

  function e() {
    for (o = 0; o < m; o++)
      if (a = s[o], (r = a.getBoundingClientRect().top) < i && 0 < r + a.offsetHeight) {
        var t = parseFloat(a.getAttribute("data-speed")) || .5,
          e = -r * t;
        a.getElementsByClassName("parallax-bg")[0].style[n] = "translate(0, " + e + "px)"
      }
  }
...
function growEvents(obj, href = window.location.href) {
  ...
  var elementOffset = $img.offset().top,
    ...
    newTop = windowHeight * .7 - 300,
    newRight = $(window).width() * .9 - imgLeft - 300;
  ...
  //                alert($(text).find('.event-img').offset().left);
  //            });
  //    $.ajax({
  //   url:href,
  //   type:'GET',
  ...
  setTimeout(function() {
    ...
    $('.slider-transition').children().not($img).remove();
...
function slideShowInit() {
  ...
  function sliderArrows() {
    $('.slideshow .arrows .arrow').on('click', function() {
      TweenMax.to($('.page.is-active i.is-animating'), 1, {
        x: '101%'
      });
